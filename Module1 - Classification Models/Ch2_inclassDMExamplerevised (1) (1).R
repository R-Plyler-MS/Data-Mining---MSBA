#### Table 2.3 
getwd()
housing.df <- read.csv("C:/Users/sergi/Documents/dmclass/DMBAdata/WestRoxbury.csv", header = TRUE)  # load data
...
# use setdiff() to find records not alre# use set.seed() to get the same partitions when re-running the R code.
set.seed(1)
...
# not already in the training setady in the training set
...
reg <- lm(TOTAL_VALUE ~ .-TAX, data = train.data) # remove variable "TAX"
